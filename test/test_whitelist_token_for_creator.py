...
def whitelist_token_for_creator():
  whitelist_args = [["Address", "0x179b6b1cb6755e31"], ["UInt64", "1"], ["UInt64", "1"], ["UFix64", "5.0"], ["UFix64", "1.0"]]
  assert send_transaction("whitelist", args=whitelist_args, signer="AsyncArtAccount")
  assert "{1: 1}" == send_script_and_return_result("getMasterMintReservation", args=[["Address", "0x179b6b1cb6755e31"]])
  # Checks that the metadata entry here is non-empty
  assert send_script("getMetadata", args=[["UInt64", "1"]])
