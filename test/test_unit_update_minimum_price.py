...
def update_minimum_price(args, signer, should_succeed, expected_result=None):
  ...
  if should_succeed:
    assert send_nft_auction_transaction("updateMinimumPrice", args=txn_args, signer=signer)
    ...
    assert not send_nft_auction_transaction("updateMinimumPrice", args=txn_args, signer=signer)
